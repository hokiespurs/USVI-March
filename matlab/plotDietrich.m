function f = plotDietrich(ortho,dietrich,tideval,sfmcompare)
CONTOURLIMS = -5:1;
%%
...
% ratio factor
axg(4);
onlywaterdietrichratio = dietrich.ratio;
onlywaterdietrichratio(dietrich.ratio==1)=nan;
pcolor(ortho.As,ortho.Xs,onlywaterdietrichratio);shading flat
colorbar
caxis([1.3 1.7]);
title('scale factor');
axis equal
...
axg(6);
plot(tideval-ortho.Zg,dietrich.ratio,'b.');
xlim([0 5]);
grid on
...
% depth correction (m)
axg(7);
pcolor(ortho.As,ortho.Xs,ortho.Zg-dietrich.corDepth);shading flat
hold on
contour(Asg,Xsg,ortho.Zg-tideval,[0 0],'m-');
colorbar
caxis([0 2]);
